...
class DiagramConstructor:
    ...
    def make_diagrams(sheet_name, file_path, folder_name, standard_deviation, show_original_values):
        ...
        non_zero_and_one_filter = ((values_2022 != 0).astype(int) + (values_2023 != 0).astype(int) + (values_2024 != 0).astype(int)) > 1
        regions, values_2022, values_2023, values_2024 = [arr[non_zero_and_one_filter] for arr in [regions, values_2022, values_2023, values_2024]]

        sorted_indices = values_2024.argsort()
        regions, values_2022, values_2023, values_2024 = [arr[sorted_indices] for arr in [regions, values_2022, values_2023, values_2024]]

        valid_data_filter = ((values_2022 != 0).astype(int) + (values_2023 != 0).astype(int) + (values_2024 != 0).astype(int)) > 1
        mean_2022, mean_2023, mean_2024 = [arr[valid_data_filter & (arr > 0)].mean() for arr in [values_2022, values_2023, values_2024]]
        std_2022, std_2023, std_2024 = [arr[valid_data_filter & (arr > 0)].std(ddof=0) for arr in [values_2022, values_2023, values_2024]]
        threshold_2022, threshold_2023, threshold_2024 = [mean + standard_deviation * std for mean, std in zip([mean_2022, mean_2023, mean_2024], [std_2022, std_2023, std_2024])]
        ...
        bars_2022 = plt.bar([i - width for i in x], [adj[0] for adj in adjusted_2022], width=width, label='2022', color='#A5A5A5')
        ...
        bars_2024 = plt.bar([i + width for i in x], [adj[0] for adj in adjusted_2024], width=width, label='2024', color='#5B9BD5')
        ...
        if show_original_values:
            for i in range(len(x)):
                if (values_2023[i] > threshold_2023 and values_2023[i] > max_non_outlier_value) or (values_2023[i] > y_max_metric):
                    plt.text(x[i], DiagramConstructor.bar_adjust(values_2023[i], threshold_2023, max_non_outlier_value)[0] + 3.5,
                             f'{int(values_2023[i]):,}'.replace(',', ' '), ha='center', va='bottom', rotation=90, fontsize=5)
                if (values_2022[i] > threshold_2022 and values_2022[i] > max_non_outlier_value) or (values_2022[i] > y_max_metric):
                    plt.text(x[i] - width, DiagramConstructor.bar_adjust(values_2022[i], threshold_2022, max_non_outlier_value)[0] + 3.5,
                             f'{int(values_2022[i]):,}'.replace(',', ' '), ha='right', va='bottom', rotation=90, fontsize=5)
                if (values_2024[i] > threshold_2024 and values_2024[i] > max_non_outlier_value) or (values_2024[i] > y_max_metric):
                    plt.text(x[i] + width, DiagramConstructor.bar_adjust(values_2024[i], threshold_2024, max_non_outlier_value)[0] + 3.5,
                             f'{int(values_2024[i]):,}'.replace(',', ' '), ha='left', va='bottom', rotation=90, fontsize=5)

        plt.axhline(y=mean_2022, color='#A5A5A5', linestyle='--', linewidth=0.8, label=f'Среднее за 2022: {int(round(mean_2022)):,}'.replace(',', ' '))
        plt.axhline(y=mean_2023, color='#ED7D31', linestyle='--', linewidth=0.8, label=f'Среднее за 2023: {int(round(mean_2023)):,}'.replace(',', ' '))
        plt.axhline(y=mean_2024, color='#5B9BD5', linestyle='--', linewidth=0.8, label=f'Среднее за 2024: {int(round(mean_2024)):,}'.replace(',', ' '))
    ...
    def run():
        ...
        if valid_sheets:
            ...
            for sheet in valid_sheets:
                DiagramConstructor.make_diagrams(sheet, file_path, folder_name, standard_deviation, show_original_values)
        else:
